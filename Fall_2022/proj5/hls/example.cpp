...
KERNEL_INIT:
  for (int aa = 0; aa < KERNEL_SIZE; aa++) {
    if (aa == 4)
      kernel[aa] = 1; // assume kernel is 3x3
    else
      kernel[aa] = 0;
  }
      ...
      KERNELW:
        for (int j = 0; j < KERNEL_HEIGHT; j++) {
          ...
          printf(
              ...
              pixel, dst[pixel], src_loc, padded_dst[src_loc], kernel_loc,
...
SENDING_OUTPUT:
  while (jj < IMAGE_SIZE) {
    ...
    // send the last bit when it reaches the image size or when it reaches the
    // max data received
    if (jj == (max_data_length - 1) || (jj == (IMAGE_SIZE - 1)))
      tmp.last = 1;
    B.write(tmp);
    jj++;
    if (tmp.last == 1)
